...
/**
 *   	\file       digitalservice_list.php
 *		\ingroup    eservices
 *		\brief      List page for digitalservice
 */
...
require_once __DIR__.'/class/digitalservice.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('eservices', 'digitalservice', 'write');
	$permissiontodelete = $user->hasRight('eservices', 'digitalservice', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("eservices")) {
	accessforbidden('Module eservices not enabled');
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Digitalservice';
	$objectlabel = 'Digitalservice';
	$uploaddir = $conf->eservices->dir_output;
...
$title = $langs->trans("Digitalservices");
//$help_url = "EN:Module_Digitalservice|FR:Module_Digitalservice_FR|ES:Módulo_Digitalservice";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/eservices/digitalservice_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/eservices/digitalservice_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendDigitalserviceRef";
$modelmail = "digitalservice";
$objecttmp = new Digitalservice($db);
...
if ($search_all) {
	...
	print '<!-- Search done like if DIGITALSERVICE_QUICKSEARCH_ON_FIELDS = '.$setupstring.' -->'."\n";
