...
function dol_move_uploaded_file($src_file, $dest_file, $allowoverwrite, $disablevirusscan = 0, $uploaderrorcode = 0, $nohook = 0, $keyforsourcefile = 'addedfile', $upload_dir = '', $mode = 0)
{
	...
	if ($reshook < 0) {	// At least one blocking error returned by one hook
		...
		if ($return) {
			...
			//modif fred 20/03/25 -- si move réussi, on fait le hook pour envoyer le nom du fichier et le répertoire pour 
			// faire le transfert que churchdata va utiliser pour l'intégration
			...
			$reshook = $hookmanager->executeHooks('moveUploadedFileSuccess', $parameters);
