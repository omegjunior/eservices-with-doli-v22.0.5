...
class FormTicket
{
	...
	public function showForm($withdolfichehead = 0, $mode = 'edit', $public = 0, $with_contact = null, $action = '', $object = null)
	{
		...
		//Fred modif ce 29/05/26
		$langs->loadLangs(array('eservices@eservices', 'other', 'mails', 'ticket'));
		...
		// Type of Ticket
		//Fred modif ce 29/05/26
		print '<tr style="display: none;"><td class="titlefield"><span class="fieldrequired"><label for="selecttype_code">'.$langs->trans("TicketTypeRequest").'</span></label></td><td>';
		$this->selectTypesTickets($type_code, 'type_code', '', 2, 'ifone', 0, 0, 'minwidth200 maxwidth500');
		print '</td></tr>';

		// Group => Category
		//Fred modif ce 29/05/26
		print '<tr style="display: none;"><td><span class="fieldrequired"><label for="selectcategory_code">'.$langs->trans("TicketCategory").'</span></label></td><td>';
		$filter = '';
		if ($public) {
			$filter = '(public:=:1)';
		}
		...
		// Severity => Priority
		//Fred modif ce 29/05/26
		print '<tr style="display: none;"><td><span class="fieldrequired"><label for="selectseverity_code">'.$langs->trans("TicketSeverity").'</span></label></td><td>';
		...
		if ($this->withtitletopic) {
			//Fred modif ce 29/05/26
			print '<tr style="display: none;"><td><label for="subject"><span class="fieldrequired">'.$langs->trans("Subject").'</span></label></td><td>';
		...
		//Fred modif ce 29/05/26
		//si public, on cache et on gère le message par js et par valeurs par défaut
		if ($this->ispublic){
			// Message
			print '<tr style="display: none;"><td><label for="message"><span class="fieldrequired">'.$langs->trans("Message").'</span></label></td><td>';
			...
			//fred modif ce 29/05/26
			// si interface back, on affiche le message pour renseigner	
			// Message
		...
		// Fred modif ce 01/06/26 : déplacement de la partie "other attributes" avant les categories et les fichiers pour que les hooks puissent injecter fields before categories and files
		// Other attributes
		$parameters = array(
		...
		if ($public && getDolGlobalString('MAIN_SECURITY_ENABLECAPTCHA_TICKET')) {
			...
			// Fred modif ce 31/05/26
			$dirModCaptcha = array_merge(array('main' => '/core/modules/security/captcha/'), is_array($conf->modules_parts['captcha'] ?? null) ? $conf->modules_parts['captcha'] : array());
