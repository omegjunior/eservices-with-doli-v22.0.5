...
$langs->loadLangs(array('eservices@eservices', 'companies', 'other', 'mails', 'ticket'));
		...
		if (!$error) {
			...
			if (!$error && $id > 0) {
				...
				if (!$error) {
					$object->db->commit();
					// Fred modif ce 30/05/26
					//attribuer catégorie demande en ligne à création depuis interface publique
					$mycat[] = 1;
					$object->setCategories($mycat);
