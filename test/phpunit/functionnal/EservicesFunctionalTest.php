...
/**
 * \file    test/phpunit/EservicesFunctionalTest.php
 * \ingroup eservices
 ...
 *
 * @package Testeservices
 */
class EservicesFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modEservices')->click();
		} else {
			// Disable the module
			$this->byHref('modEservices')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/eservices/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('eservices/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/eservices/admin/about.php');
		$this->authenticate();
		return $this->assertContains('eservices/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('eserviceswidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modEservices_EservicesTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modEservices_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
