...
if (empty($reshook) && !empty($object->table_element) && isset($extrafields->attributes[$object->table_element]['label']) && is_array($extrafields->attributes[$object->table_element]['label'])) {
	...
	// on fait cette suppression dans tous les cas de view et build sauf en cas de update_extra où c'est déjà fait dans actions_esevices.class.php
	if ($action_original !== 'update_extras'){
		//récupérer la valeur de l'attribut 'eservice' de l'objet qu'on veut afficher
		$eservice_value = (isset($object->array_options["options_eservicelie"]) ? $object->array_options["options_eservicelie"] : '');
		//dol_syslog('fred admin ici extrafieldsview value eservicelie : '.$eservice_value);
		//on supprime les extrafields non nécessaires
		// Récupérez le tableau à traiter
		$labels = $extrafields->attributes[$object->table_element]['label'];	
		// Parcourez le tableau des labels
		foreach ($labels as $key => $value) {
			// Vérifiez si la clé commence par un entier suivi d'un underscore
			if (preg_match('/^(\d+)_/', $key, $matches)) {
				// Si l'entier dans la clé est différent de $eservice_value, on supprime l'élément
				if ((int)$matches[1] !== (int)$eservice_value) {
					unset($labels[$key]);
				}
			}
		}
		// Après la suppression, on remet à jour l'élément du tableau
		$extrafields->attributes[$object->table_element]['label'] = $labels;
	}
