...
<script>
	jQuery(document).ready(function() {
		...
		jQuery("#label").keyup(function() {
			...
			$("#attrname").val( $("#listeeservices").val()+'_'+ ($(this).val().replace(/[^a-zA-Z0-9_]/g, '').toLowerCase()) );
		});

		// Gérer l'ajout/suppression de la classe 'fieldrequired' en fonction de la checkbox
		jQuery("#required").change(function () {
			var isChecked = jQuery(this).is(":checked");
			if (isChecked) {
				jQuery("tr.extra_default_value td:first-child").addClass("fieldrequired");
			} else {
				jQuery("tr.extra_default_value td:first-child").removeClass("fieldrequired");
			}
   		});
		 // Vérification à l'initialisation de la page
...
<!-- liste des e-services soumis à la demande -->
<?php 
	//array de liste eservices
	$sql  = "SELECT DISTINCT rowid, ref, label ";
	$sql .= "FROM ".MAIN_DB_PREFIX."eservices_digitalservice ";
	$sql .= " WHERE entity = ".((int) $conf->entity);
	$resql = $db->query($sql);
	if ($resql) {
		$listeeservices = array();
		$num = $db->num_rows($resql);
		$i = 0;
		if ($num > 0) {
			while ($i < $num) {
				$objp = $db->fetch_object($resql);
				$listeeservices[$objp->rowid] = $objp->label;
				$i++;
			}
		}
	} else{
		//une liste eservices vides
		$listeeservices = array('1' => '', '2' => '');
	}
	$db->free($resql);
	//liste des e-services
	print '<tr><td class="fieldrequired">'.$langs->trans("listeeservices").'</td><td class="valeur">';
	print $form->selectarray('listeeservices', $listeeservices, 'ifone', 0, 0, 0, '', 0, 0, 0, '', '', 1);
	print '</td></tr>';
?>
<!-- Label -->
...
<!-- Code -->
<tr>
    <td class="fieldrequired" style="display:none;">
        <?php echo $langs->trans("AttributeCode"); ?>
    </td>
    <td class="valeur" style="display:none;">
        <input type="hidden" name="attrname" id="attrname" size="10" value="<?php echo GETPOST('attrname', 'alpha'); ?>" pattern="\w+">
        <span class="opacitymedium">(<?php echo $langs->trans("AlphaNumOnlyLowerCharsAndNoSpace"); ?>)</span>
    </td>
</tr>
<!-- Type -->
...
<!-- Language file -->
<tr><td class="titlefield"><?php echo $langs->trans("LanguageFile"); ?></td><td class="valeur"><input type="text" id="langfile" name="langfile" class="minwidth200" value="<?php echo dol_escape_htmltag(GETPOST('langfile', 'alpha') ?: 'eservices@eservices'); ?>"></td></tr>
<!-- Computed Value -->
