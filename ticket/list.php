...
/**
 *    \file     htdocs/custom/eservices/ticket/list.php
 *    \ingroup	ticket
...
$langs->loadLangs(array('eservices@eservices', "companies", "other", "projects", "contracts"));
...
$url_page_current = DOL_URL_ROOT.'/custom/eservices/ticket/list.php';
...
if ($num == 1 && getDolGlobalString('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/custom/eservices/ticket/card.php?id='.$id);
	exit;
}
...
$url = DOL_URL_ROOT.'/custom/eservices/ticket/card.php?action=create&mode=init'.($socid ? '&socid='.$socid : '').($projectid ? '&origin=projet_project&originid='.$projectid : '');
if (!empty($socid)) {
	$url .= '&socid='.$socid;
}
$newcardbutton = '';
...
$moreforfilter.= '</div>';*/
//fred modif -- essayer de néttoyer le arrayfields des champs non souhaités
$parameters = array('arrayfields' => &$arrayfields);
