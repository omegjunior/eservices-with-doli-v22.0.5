...
/**
 *   \file       htdocs/custom/eservices/ticket/card.php
 *   \ingroup    ticket
...
$langs->loadLangs(array('eservices@eservices', "companies", "other", "ticket"));
...
//fred modif ce 30/05/26 : récupérer l'action d'origine pour faire des tests dans extrafields_view et éviter répétion de suppression de champs extrafields
$action_original = !empty($action)? $action : '';
...
$url_page_current = DOL_URL_ROOT.'/custom/eservices/ticket/card.php';
...
if (empty($reshook)) {
	...
	$backurlforlist = DOL_URL_ROOT . '/custom/eservices/ticket/list.php';
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = DOL_URL_ROOT . '/custom/eservices/ticket/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
	...
	if ((($action == 'add' && GETPOST('save', 'alpha')) || ($action == 'update' && $object->status < Ticket::STATUS_CLOSED)) && $permissiontoadd) {
		...
		if (!$error) {
			...
			// fed modif ce 30/05/26
			//fixer par avance le modèle à utiliser par ce ticket pour générer en fonction du eservicelie
			include_once DOL_DOCUMENT_ROOT.'/custom/eservices/class/digitalservice.class.php'; 
			$digitalservice = new Digitalservice($db);
			//dol_syslog('fred admin digit : '.$object->array_options['options_eservicelie']);
			$digitalservice->fetch($object->array_options['options_eservicelie']);
			$object->model_pdf = 'generic_ticket_odt:'.DOL_DATA_ROOT.'/eservices/doctemplates/demandes/'.(str_replace(' ', '', strtolower($digitalservice->label))).'.odt';
	...
	if ($action == 'confirm_delete_ticket' && GETPOST('confirm', 'alpha') == "yes" && $permissiontodelete) {
		if ($object->fetch(GETPOSTINT('id'), '', GETPOST('track_id', 'alpha')) >= 0) {
			if ($object->delete($user) > 0) {
				...
				header("Location: " . DOL_URL_ROOT . "/custom/eservices/ticket/list.php");
				exit;
			} else {
				$langs->load("errors");
	...
	if ($action == "update_extras" && $permissiontoeditextra) {
		...
		if (!$error) {
			// Fred modif ce 30/05/26
			if($attribute_name == 'eservicelie'){
				// s'il n 'y a pas d'erreur sur l'extrafields, alors modifier l'objet
				//fixer par avance le modèle à utiliser par ce ticket pour générer en fonction du eservicelie
			...
			if ($result < 0) {
				...
				if($attribute_name == 'eservicelie'){
					$model_pdf = (str_replace(' ', '', strtolower($digitalservice->label))).'.odt';
					$sql = "UPDATE ".MAIN_DB_PREFIX."ticket SET model_pdf = ".$model_pdf;	
					$sql = " WHERE rowid = ".$object_id;
					$resql = $db->query($sql);
					if ($resql < 0) {
						$error++;
					}					
...
if ($action == 'create' || $action == 'presend') {
	...
	if (!empty($res) && $res > 0) {
		...
		$linkback = '<a href="'.DOL_URL_ROOT.'/custom/eservices/ticket/list.php?restore_lastsearch_values=1"><strong>'.$langs->trans("BackToList").'</strong></a> ';
		...
		*/
		...
		// Other attributes
		include DOL_DOCUMENT_ROOT.'/custom/eservices/core/tpl/extrafields_view.tpl.php';
		...
		if (getDolGlobalString('TICKET_SHOW_MESSAGES_ON_CARD')) {
			...
			$messagingUrl = DOL_URL_ROOT.'/custom/eservices/ticket/agenda.php?track_id='.$object->track_id;
