...
/**
 *    \file       htdocs/custom/eservices/ticket/index.php
 *    \ingroup    ticket
 */
...
if ($user->hasRight('ticket', 'read')) {
	...
	if ($result) {
		...
		print '<a href="'.DOL_URL_ROOT.'/custom/eservices/ticket/list.php?search_fk_statut[]='.Ticket::STATUS_NOT_READ.'" title="'.$langs->trans("FullList").'">';
		...
		if ($num > 0) {
			while ($i < $num) {
				...
				print '<a href="'.DOL_URL_ROOT.'/custom/eservices/ticket/card.php?track_id='.$objp->track_id.'" title="'.dolPrintHTMLForAttribute($objp->subject).'">'.dol_trunc($objp->subject, 30).'</a>';
				print "</td>\n";
